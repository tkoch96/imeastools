import numpy as np, csv, socket, struct, os, re, geopy.distance
from bisect import bisect_left
from imeastools.constants import *
...
def temporal_split_kusto_query(cluster, database, table, query, total_time_hours, 
		start_ago=24, out_dir=None, pre_defs=None, time_keyword="['time']", delta=1):
	# Executes a kusto query, dividing the query into time bins
	# kusto limits size of returned results, so we need to temporally split the query to get all the data
	...
	for tbi in range(len(time_blocks) - 1):
		...
		kusto_response = KUSTO_CLIENT.execute(KUSTO_DATABASE, this_timestep_query)
...
def load_cities_data(data_dir = ".", us_states=False):
	...
	with open(os.path.join(data_dir, "data", "world_cities.csv"), 'r',encoding='latin1') as f:
		for i, row in enumerate(f):
			...
			try:
				...
				try:
					...
					# Ignore unpopular cities with common names
